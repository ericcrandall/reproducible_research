...
1.  Go to <https://github.com/ericcrandall/reproducible_research_practical>
2.  Click "Fork" in the upper right-hand corner and follow dialogue
    prompts to create fork in your account
3.  If you want to create a new git repository for yourself, the best approach
    to create it on on the DVCS (Github, Bitbucket, etc.), and then copy the URL
...
2.  Create project from: Choose "Version Control"
    ...
    2.  Input the url of this repository
        1.https://github.com/*yourusername*/reproducible_research_practical
    ...
    5.  (browse to where you want to put it in the "create project as
        subdirectory of:")
    6.  So it should be in `yourhome/github/reproducible_research_practical`
...
-   Now we want to `push` these up to Github. Easy - just use the green arrow, or at the command line, `git push`. 
   -  If you want to get all fancy, you could type `git push origin main`. `origin` is the name of the remote site (in theory you could have more than one) and `main` is the name of the branch.
...
-   You can treat them as very specific names for different commits. You can usually just type the first 8 letters of the hash to identify a specific commit.
...
-   Example: `*.html`
-   Useful for ignoring large data files, or other things you want to stay local (e.g. for privacy reasons)
